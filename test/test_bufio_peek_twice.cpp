...
static void on_second_peek(uve_request_t* request, int status, uv_buf_t* buf) {
  called[1] = true;
  ASSERT_EQ(0, status);
...
static void on_connection(uv_stream_t* server, int status) {
  ...
  ASSERT_EQ(0, r);
  r = uve_bufio_request(reader, uve_request_peek(4, client, on_second_peek));
  ASSERT_EQ(0, r);
}
...
static void on_sender_connect(uv_connect_t* req, int status) {
  ...
  wr->data = sender;
  uv_buf_t buf = uv_buf_init("12345678", 8);
  r = uv_write(wr, (uv_stream_t*)sender, &buf, 1, on_sender_write);
}

TEST(bufio, peek_twice) {
