...
static void on_first_peek(uve_request_t* request, int status, uv_buf_t* buf) {
  called[0] = true;
...
static void on_second_read(uve_request_t* request, int status, uv_buf_t* buf) {
  called[1] = true;

  ASSERT_EQ(0, status);
  ASSERT_EQ(3, buf->len);
  ASSERT_EQ(0, memcmp("123", buf->base, 3));
...
static void on_third_peek(uve_request_t* request, int status, uv_buf_t* buf) {
  called[2] = true;
...
static void on_connection(uv_stream_t* server, int status) {
  ...
  r = uve_bufio_request(reader, uve_request_peek(4, client, on_first_peek));
  ASSERT_EQ(0, r);
  r = uve_bufio_request(reader, uve_request_read(3, client, on_second_read));
  ASSERT_EQ(0, r);
  r = uve_bufio_request(reader, uve_request_peek(2, client, on_third_peek));
  ASSERT_EQ(0, r);
}
